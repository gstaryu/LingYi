# ANSI/GBK 编码版启动脚本（PowerShell 5.1 直接运行中文不乱码）
# UTF-8 版本见 start.ps1；两者逻辑完全一致，任选其一运行
...
if (-not (Test-Path .env)) {
    Write-Host "未找到 .env，请先配置 DASHSCOPE_API_KEY（参考 README）" -ForegroundColor Yellow
}
